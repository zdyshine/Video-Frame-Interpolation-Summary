...
def main():
    ...
    model = EDSC.Network(isMultiple=True).cuda() # EDSC_m
    checkpoint = torch.load('./pretrained_model/EDSC_m.ckpt', map_location=torch.device('cpu'))
    ...
    for d in tqdm(dirs):
        ...
        # inference
        time_torch = torch.ones(
            (1, 1, int(img0.shape[2] / 2), int(img0.shape[3] / 2))) * 0.1
        pred = model([img0, img1, time_torch.cuda()])[0]
