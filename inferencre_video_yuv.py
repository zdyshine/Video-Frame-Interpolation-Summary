...
parser.add_argument('--video', dest='video', type=str, default='../../data/source/test.mp4')
...
# model_path = 'checkpoint/kernelsize_5/ckpt.pth'
model_path = 'checkpoint/best_checkpoints/adacof/model_epoch046.pth' # my best
# model_path = './output_adacof_train_withVimeo/checkpoint/model_epoch046.pth'
print('=========> load from: ', model_path)
checkpoint = torch.load(model_path, map_location=torch.device('cpu'))
# checkpoint = torch.load('checkpoint/model_epoch045.pth', map_location=torch.device('cpu'))
model.load_state_dict(checkpoint['state_dict'])
model.eval()
model.to(device)
# python inference_video.py --ext=yuv --exp=1 --video=./test.mp4
# ffmpeg -f rawvideo -vcodec rawvideo -s 1920x1080 -pix_fmt yuv420p -r 50 -i ./test_2X_50fps_AdaCoF.yuv -c:v libx264 -crf 12 ./test_2X_50fps_AdaCoF.mp4 
...
if args.png:
    ...
    if args.output is not None:
        ...
        vid_out_name = '{}_{}X_{}fps_AdaCoF.{}'.format(video_path_wo_ext, (2 ** args.exp), int(np.round(args.fps)),
                                                       args.ext)
        print('========>', vid_out_name)
    #vid_out = cv2.VideoWriter(vid_out_name, fourcc, args.fps, (w, h))
    vid_out = open(vid_out_name, 'wb')
...
def clear_write_buffer(user_args, write_buffer):
    ...
    while True:
        ...
        item = cv2.cvtColor(item, cv2.COLOR_RGB2YUV_I420)
        if item is None:
            break
        if user_args.png:
...
while True:
    ...
    if ssim < 0.5:
        ...
        # images = torch.stack([I0, I0, I1, I1], dim=1)
...
# python inference_video.py --exp=1 --video=../../data/source/test.mp4
