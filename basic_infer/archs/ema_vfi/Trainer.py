...
from archs.ema_vfi.model.loss import *

from archs.ema_vfi.config import *
...
class Model:
    ...
    def load_model(self, model_path, name=None, rank=0):
        def convert(param):
            return {
                k.replace("module.", ""): v
        ...
        if rank <= 0:
            ...
            self.net.load_state_dict(convert(torch.load(model_path)))
    ...
    def inference(self, img0, img1, TTA=False, timestep=0.5, fast_TTA=False):
        ...
        if fast_TTA:
            imgs_ = imgs.flip(2).flip(3)
            input = torch.cat((imgs, imgs_), 0)
            preds = self.net(input, timestep=timestep)[-1]
            return (preds[0] + preds[1].flip(1).flip(2)).unsqueeze(0) / 2.

        pred = self.net(imgs, timestep=timestep)[-1]
        if TTA == False:
            return pred
        else:
            pred2 = self.net(imgs.flip(2).flip(3), timestep=timestep)[-1]
