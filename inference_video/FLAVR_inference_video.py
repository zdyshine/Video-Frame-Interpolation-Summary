...
from pytorch_msssim.pytorch_msssim import ssim_matlab
...
from model.FLAVR_arch import UNet_3D_3D  # get model
...
if args.png:
    ...
    if args.output is not None:
        ...
        vid_out_name = '{}_{}X_{}fps_FLAVR.{}'.format(video_path_wo_ext, (2 ** args.exp), int(np.round(args.fps)), args.ext)
        print('========>', vid_out_name)
...
def make_inference(I0, I1, exp):
    global model
    middle = model([I0, I0, I1, I1])[0]
    middle = torch.clamp(middle, 0, 1)
    if exp == 1:
        return [middle]
    first_half = make_inference([I0, I0, middle, middle], exp=exp - 1)[0]
    first_half = torch.clamp(first_half, 0, 1)
    second_half = make_inference([middle, middle, I1, I1], exp=exp - 1)[0]
